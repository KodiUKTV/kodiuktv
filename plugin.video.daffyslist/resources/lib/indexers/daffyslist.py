...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'aHR0cDovL2RhZmZ5c2xpc3QuY29tL3htbC9saXN0cy9tYWluL21haW4ueG1s'.decode('base64')
            self.list = self.daffyslist_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.daffyslist_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.daffyslist_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            link = 'aHR0cDovL2RhZmZ5c2xpc3QuY29tL3htbC9saXN0cy9tYWluL3NlYXJjaC54bWw='.decode('base64')
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.daffyslist_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['daffyslist TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def daffyslist_list(self, url, result=None):
        try:
